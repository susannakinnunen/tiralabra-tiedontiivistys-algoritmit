...
class TestHuffmanCodingSmallFiles(unittest.TestCase):
    def setUp(self):
        ...
        self.huffman_compress_small = HuffmanCodingCompress(self.path_small, "test")
        self.huffman_decompress = HuffmanCodingDecompress(self.path_small)


        FileForTesting() # Creates a bit bigger test_file.txt
        self.path_big_nonsense = os.path.join(os.getcwd(), "test_big_nonsense.txt")
        self.huffman_compress_big_nonsense = HuffmanCodingCompress(self.path_big_nonsense, "test")

#From here tests are done with the big nonsense file.
    def test_frequency_table(self):
        """Test create_frequency_table method"""
        frequence_table = self.huffman_compress_big_nonsense.create_frequence_table()
    ...
    def test_create_minimum_heap(self):
        ...
        self.huffman_compress_big_nonsense.create_frequence_table()
        self.huffman_compress_big_nonsense.create_minimum_heap()
        self.assertEqual(str(heapq.heappop(self.huffman_compress_big_nonsense.heap)), "(:1")
        self.assertEqual(str(heapq.heappop(self.huffman_compress_big_nonsense.heap)), '":3')
        self.assertEqual(str(heapq.heappop(self.huffman_compress_big_nonsense.heap)), "Ä:68")


    def test_create_a_merged_node(self):
        self.huffman_compress_big_nonsense.create_frequence_table()
        self.huffman_compress_big_nonsense.create_minimum_heap()
        merged_node = self.huffman_compress_big_nonsense.create_merged_node()
    ...
    def test_create_huffman_tree(self):
        ...
        self.huffman_compress_big_nonsense.create_frequence_table()
        self.huffman_compress_big_nonsense.create_minimum_heap()
        huffman_tree = self.huffman_compress_big_nonsense.create_huffman_tree()
...
#From here tests are done with a very short strings (very small files).
    def test_create_codes(self):
        ...
        self.huffman_compress_small.create_frequence_table()
        self.huffman_compress_small.create_minimum_heap()
        self.huffman_compress_small.create_huffman_tree()

        codes = self.huffman_compress_small.create_codes()
    ...
    def test_create_encoded_string(self):
        ...
        self.huffman_compress_small.create_minimum_heap()
        self.huffman_compress_small.create_huffman_tree()
        self.huffman_compress_small.create_codes()
        encoded_string = self.huffman_compress_small.create_encoded_string()
    ...
    def test_binary_file_length_and_correctness(self):
        ...
        self.huffman_compress_small.create_codes()
        encoded_string = self.huffman_compress_small.create_encoded_string()
        binary_string = self.huffman_compress_small.write_remaining_bits(encoded_string)
    ...
    def test_remove_filling_bits(self):
        ...
        self.huffman_compress_small.create_huffman_tree()
        self.huffman_compress_small.create_codes()
        binary_string_before_compression = self.huffman_compress_small.create_encoded_string()
        binary_string_after_compression_filling = self.huffman_decompress.get_binary_string_from_compressed_file()

        binary_string_after_compression_no_filling = self.huffman_decompress.remove_filling_bits(binary_string_after_compression_filling)

        self.assertEqual(binary_string_after_compression_no_filling, binary_string_before_compression)

    def test_decode_string(self):
        self.huffman_compress_small.compress()
        
        binary_string_after_compression_filling = self.huffman_decompress.get_binary_string_from_compressed_file()
        binary_string_after_compression_no_filling = self.huffman_decompress.remove_filling_bits(binary_string_after_compression_filling)
        string = self.huffman_decompress.decode_string(binary_string_after_compression_no_filling)
